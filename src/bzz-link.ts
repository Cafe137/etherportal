 ...
 * @param req
 */
export function requestFilter(_pathname: string, req: Request): boolean {
 ...
 * @param next
 */
export function errorHandler(err: Error, _req: Request, res: Response, next: (e: Error) => void): void {
  if (res.headersSent) {
    next(err)
